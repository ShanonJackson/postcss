...
type PrepareHook = Box<
    dyn Fn(&mut PostcssResult) -> Result<Option<Arc<dyn Plugin>>, ProcessorError> + Send + Sync,
>;
...
impl PluginBuilder {
    ...
    where
        F: Fn(&Declaration, &mut PostcssResult) -> Result<(), ProcessorError>
            + Send
            + Sync
            + 'static,
    {
