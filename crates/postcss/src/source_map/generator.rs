    ...
    pub fn generate(&mut self) -> Result<MapResult, PreviousMapError> {
        ...
        if !map_enabled {
            if let Some(root) = &self.root {
                let mut builder =
                    |chunk: &str, _node: Option<&NodeRef>, _ty: Option<&'static str>| {
                        css.push_str(chunk);
                    };
        ...
        if let Some(root) = &self.root {
            let mut builder = |chunk: &str,
                               node: Option<&NodeRef>,
                               string_type: Option<&'static str>| {
                if let Some(node_ref) = node {
                    ...
                    if string_type != Some("end") {
                        let (source_id, original_line, original_column) =
        ...
        let sourcemap = {
            ...
            String::from_utf8(buf).map_err(|_| {
                PreviousMapError("Serialized source map is not valid UTF-8".to_string())
            })?
        };
