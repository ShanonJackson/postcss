        ...
        for (i, token) in tokens.iter().enumerate() {
            match token.kind {
                ...
                TokenKind::Comment => {
                    ...
                    let prev_safe =
                        prev_kind.is_none() || matches!(prev_kind, Some(TokenKind::Space));
                    let next_safe =
                        next_kind.is_none() || matches!(next_kind, Some(TokenKind::Space));
            ...
            match token.kind {
                ...
                TokenKind::Colon if brackets == 0 => {
                    if prev.is_none() {
                        ...
                        .map(|t| {
                            t.kind == TokenKind::Word && t.value.eq_ignore_ascii_case("progid")
                        })
                        .unwrap_or(false)
                    {
                        // continue scanning
    ...
    fn atrule(&mut self, token: Token) -> Result<(), CssSyntaxError> {
        ...
        while !self.tokenizer.end_of_file() {
            ...
            match kind {
                ...
                TokenKind::OpenCurly if !brackets.is_empty() => {
        ...
        while let Some(token) = tokens.first() {
            ...
            if matches!(kind, TokenKind::Colon) {
                ...
                if matches!(kind, TokenKind::Word)
                    && token.value.chars().any(|c| c.is_alphanumeric())
                {
