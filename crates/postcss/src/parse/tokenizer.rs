    ...
    pub fn next_token(&mut self, ignore_unclosed: bool) -> Result<Option<Token>, CssSyntaxError> {
        ...
        match code {
            ...
            COLON => {
                let start = self.pos;
                current_token =
                    Token::new(TokenKind::Colon, ":".to_string(), Some(start), Some(start));
                self.pos += 1;
                return Ok(Some(current_token));
            }
            SEMICOLON => {
            ...
            _ => {
                if code == SLASH && self.bytes.get(self.pos + 1) == Some(&ASTERISK) {
                    ...
                    current_token =
                        Token::new(TokenKind::Comment, value, Some(self.pos), Some(next_index));
                    ...
                    current_token =
                        Token::new(TokenKind::Word, value, Some(self.pos), Some(next - 1));
