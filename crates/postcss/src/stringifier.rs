            ...
            let value = match detect_key {
                ...
                other => {
                    ...
                    if method_name == "rawAfter" {
                        ...
                        self.raw_from_children(&root, own)
    ...
    fn raw_value(&self, node: &NodeRef, prop: &str) -> String {
        let (value, raw_pair) = {
            ...
            let (value, raw) = match (&borrowed.data, prop) {
                (NodeData::Rule(data), "selector") => {
                    (Some(&data.selector), borrowed.raws.get("selector"))
                }
                (NodeData::AtRule(data), "params") => {
                    (Some(&data.params), borrowed.raws.get("params"))
                }
                (NodeData::Declaration(data), "value") => {
                    (Some(&data.value), borrowed.raws.get("value"))
                }
                _ => (None, None),
            };
