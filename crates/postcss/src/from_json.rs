...
fn parse_source(value: &Value, inputs: &[InputRef]) -> Result<SourceFields, FromJsonError> {
    let object = value.as_object().ok_or_else(|| {
        FromJsonError::InvalidFormat("source metadata must be an object".to_string())
    })?;
    ...
    match value {
        ...
        Some(Value::Object(object)) => {
            ...
            let column = object
                .get("column")
                .and_then(|v| v.as_u64())
                .ok_or(FromJsonError::MissingField("source.column"))?
                as u32;
...
fn parse_raws(value: &Value) -> Result<RawData, FromJsonError> {
    let object = value.as_object().ok_or_else(|| {
        FromJsonError::InvalidFormat("raws metadata must be an object".to_string())
    })?;
    ...
    match value {
        ...
        Value::Object(object) => {
            if let (Some(value_field), Some(raw_field)) = (object.get("value"), object.get("raw")) {
                if let (Some(value_text), Some(raw_text)) =
                    (value_field.as_str(), raw_field.as_str())
                {
